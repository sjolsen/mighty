...
;;;
(defun nullablep (L)
  (labels ((should-short-circuit (L child-delta)
             (etypecase L
               (alternate-language child-delta)
               (catenate-language (not child-delta))))
           (do-short-circuit (L new-delta changed)
             (let* ((new-changed (setf->changed (delta-cache L) new-delta))
                    (changed (or changed new-changed))
                    (parent (delta-visited L)))
               (do-continuation parent changed)))
           (delta-would-recurse (L)
           ...
           (delta-start (L caller changed)
             (cond ((not (delta-would-recurse L))
                    (do-continuation caller changed))
                   ((delta-would-recurse (right L)) ;; Optimization - only recurse if necessary
                    (setf (delta-continuation L) ':other-right
                          (delta-visited L) caller)
                    (delta-start (left L) L changed))
                   (t
                    (setf (delta-continuation L) ':other-left
                          (delta-visited L) caller)
                    (delta-start (right L) L changed))))
           (do-continuation (L changed)
             (if L
                 (ecase (delta-continuation L)
                   (:other-right
                    (setf (delta-continuation L) ':combine)
                    (let ((child-delta (delta-base (left L))))
                      (if (should-short-circuit L child-delta)
                          (do-short-circuit L child-delta changed)
                        (delta-start (right L) L changed))))
                   (:other-left
                    (setf (delta-continuation L) ':combine)
                    (let ((child-delta (delta-base (right L))))
                      (if (should-short-circuit L child-delta)
                          (do-short-circuit L child-delta changed)
                        (delta-start (left L) L changed))))
                   (:combine
                    (let* ((new-delta (etypecase L
                                        (alternate-language (or (delta-base (left L))
                                                                (delta-base (right L))))
                                        (catenate-language (and (delta-base (left L))
                                                                (delta-base (right L))))))
                           (new-changed (setf->changed (delta-cache L) new-delta))
                           (changed (or changed new-changed)))
                      (do-continuation (delta-visited L) changed))))
                 changed))
           (delta (L)
             (let ((changed (delta-start L nil nil)))
               (values (delta-base L) changed))))
